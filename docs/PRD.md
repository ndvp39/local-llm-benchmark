...
- **FR-2b.** A separate `config.plumbing_test_model` entry (a small model + aggressive quantization, default Q2) **MUST** also be defined for the §3.10 pipeline plumbing test.
...
- **FR-PT-1.** Before any run against a model listed in `config.target_models`, the SDK **MUST** execute a **plumbing test** against `config.plumbing_test_model` (small model + Q2 by default) that exercises the full pipeline: HF download → AirLLM `mmap` allocation → metric collection (TTFT, TPOT, peak RAM/VRAM) → manifest write → cleanup.
