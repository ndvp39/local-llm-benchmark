...
- **SC-1 — Baseline behaviour must be captured visibly.** Running an oversized target model through the Direct back-end on the bare hardware MUST produce a captured, per-run manifest — whether the observed outcome is OOM, swap-thrash, unacceptable latency, or (with library-level rescue paths engaged) a slow-but-successful run. **The Direct back-end MUST attempt the naive full-RAM load path** (`device_map=None`, `low_cpu_mem_usage=False`) — not the silent accelerate disk-offload path that `device_map="auto"` triggers, because the offload path collapses the Direct-vs-AirLLM comparison the M3 milestone depends on. On Windows a naive oversized fp16 allocation is killed at OS signal level (exit 139) before Python's `try/except` can catch, so a Python-level **pre-flight RAM check** guards the load and raises `MemoryError` with a structured message when cached weights exceed 90 % of `psutil.virtual_memory().available`. As delivered on 2026-07-01 (T-2.11): the naive-load pre-flight fires for Llama-3-8B-Instruct fp16 and Qwen2-7B-Instruct fp16 on the 7.8 GB reference box, producing the canonical `baseline_<label>_<ts>_failure.json` manifests that anchor the SC-1 requirement. The Act-1 accelerate-offload success (`baseline_llama3-8b-fp16_20260630T221156Z.json`) is kept as a complementary Direct+offload data point for the M3/M6 tradeoff analysis (see prompts_book §11).
